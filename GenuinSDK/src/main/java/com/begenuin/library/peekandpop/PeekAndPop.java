package com.begenuin.library.peekandpop;
...
import com.begenuin.library.peekandpop.model.LongHoldView;
...
public class PeekAndPop {
    ...
    protected void init() {
        ...
        this.audioManager = (AudioManager) builder.activity.getSystemService(Context.AUDIO_SERVICE);
    ...
    private void initialiseViewTreeObserver() {
        peekView.getViewTreeObserver().addOnGlobalLayoutListener(this::initialisePeekViewOriginalPosition);
    }
    ...
    protected void initialiseGestureListener(@NonNull View view, int position, boolean isEnabled) {
        view.setOnTouchListener(new PeekAndPopOnTouchListener(position, isEnabled));
        // onTouchListener will not work correctly if the view doesn't have an
        // onClickListener set, hence adding one if none has been added.
        if (!view.hasOnClickListeners()) {
            view.setOnClickListener(v -> {
...
//        }
    ...
    private boolean isAudioFocusGain = false;

    private void setAudioFocusGain(){
    ...
    private void setAudioFocusRelease(){
        if (isAudioFocusGain && audioManager != null && audioFocusRequest != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    private void blurBackground() {
        peekLayout.setBackgroundColor(builder.activity.getResources().getColor(R.color.black_opacity80, null));
    ...
    public void addLongClickView(View view, int position, boolean isEnabled) {
        initialiseGestureListener(view, position, isEnabled);
    }
    ...
    protected class PeekAndPopOnTouchListener implements View.OnTouchListener {
        ...
        public PeekAndPopOnTouchListener(int position, boolean isEnabled) {
            this.position = position;
        ...
        public boolean onTouch(final View view, MotionEvent event) {
            ...
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                cancelPendingTimer(view);
                pop(view, position);
                Utility.showLog("TAG", event.getAction() + " Action " + peekShown);
            }
